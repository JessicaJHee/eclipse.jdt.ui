/*******************************************************************************
 * Copyright (c) 2019, 2020, 2023 IBM Corporation and others.
 *
 ...
 *     Red Hat Inc. - refactored InitializeFinalFieldProposal to jdt.core.manipulation
 *******************************************************************************/
...
public class InitializeFinalFieldProposalOperation extends CompilationUnitRewriteOperation {
	...
	public InitializeFinalFieldProposalOperation(IProblemLocationCore problem, ASTNode astNode, IVariableBinding variableBinding) {
		fAstNode= astNode;
		fProblem= problem;
	...
	public InitializeFinalFieldProposalOperation(IProblemLocationCore problem, ASTNode astNode, IVariableBinding variableBinding, int updateType) {
		fAstNode= astNode;
		fProblem= problem;
		fVariableBinding= variableBinding;
		...
		switch (fupdateType) {
			case UPDATE_AT_CONSTRUCTOR:
				if (fAstNode != null
					...
					doInitRecordComponentsInConstructor(linkedModel);
				}
				doInitFieldInConstructor(linkedModel);
				break;
			case UPDATE_AT_DECLARATION:
				doInitField(cuRewrite, linkedModel);
				break;
			case UPDATE_CONSTRUCTOR_NEW_PARAMETER:
				doUpdateConstructorWithParameter(cuRewrite, linkedModel);
				break;
			default:
				break;
		}
	...
	private ASTRewrite doInitField(CompilationUnitRewrite cuRewrite, LinkedProposalModelCore linkedModel) {
		...
		if (type.isPrimitive()) {
			...
			if (type.isParameterizedType()) {
				newType= createParameterizedType(ast, type, cuRewrite);
			} else {
		...
		linkedModel.setEndPosition(rewrite.track(expression)); // set cursor after expression statement
		return rewrite;
	}

	private Type createParameterizedType(AST ast, ITypeBinding typeBinding, CompilationUnitRewrite cuRewrite) {
		if (typeBinding.isParameterizedType() && !typeBinding.isRawType()) {
			...
			for (int i= 0; i < typeBinding.getTypeArguments().length; i++) {
				...
				Type argType= createParameterizedType(ast, typeArg, cuRewrite); // recursive call
			...
			if (!typeBinding.isTypeVariable()) {
				if (typeBinding.isWildcardType()) {
					ImportRewrite importRewrite= cuRewrite.createImportRewrite((CompilationUnit) fAstNode.getRoot(), true);
	...
	private ASTRewrite doInitRecordComponentsInConstructor(LinkedProposalModelCore linkedModel) {
		...
		for (MethodDeclaration md : cv.getNodes()) {
			...
			linkedModel.setEndPosition(rewrite.track(assignment)); // set cursor after expression statement
		}
		return rewrite;
	}
	...
	private ASTRewrite doUpdateConstructorWithParameter(CompilationUnitRewrite cuRewrite, LinkedProposalModelCore linkedModel) {
		String variableName= fProblem.getProblemArguments()[0];
		...
		String newName= StubUtility.suggestArgumentName(cuRewrite.getCu().getJavaProject(), variableName, excludedNames);
	...
	private ASTRewrite doInitFieldInConstructor(LinkedProposalModelCore linkedModel) {
		String variableName= fProblem.getProblemArguments()[0];
		FieldDeclaration field= getFieldDeclaration(variableName);
		if (field == null) {
			return null;
